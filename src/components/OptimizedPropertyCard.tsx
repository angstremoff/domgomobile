...
const OptimizedPropertyCard = ({ property, onPress, darkMode = false }: PropertyCardProps) => {
...
// Функция сравнения для предотвращения ненужных ререндеров
const areEqual = (prevProps: PropertyCardProps, nextProps: PropertyCardProps) => {
  // Проверяем только основные свойства, которые влияют на рендеринг
  return (
    prevProps.property.id === nextProps.property.id &&
    prevProps.property.price === nextProps.property.price &&
    prevProps.darkMode === nextProps.darkMode &&
    // Избранное сравнивать здесь не можем, так как useFavorites - это хук,
    // и он будет вызываться в мемоизированной функции сравнения
    // Компонент обновится при изменении статуса избранного благодаря контексту
    JSON.stringify(prevProps.property.images) === JSON.stringify(nextProps.property.images)
  );
};

// Мемоизация компонента
const MemoizedPropertyCard = memo(OptimizedPropertyCard, areEqual);

export default MemoizedPropertyCard;
