...
```
Скрипт соберёт `bundleRelease` и скопирует файл на рабочий стол (`DomGoMobile-<версия>-release.aab`) для загрузки в Google Play Console.

### Публикация в Google Play
- Подготовьте и выложите **политику конфиденциальности**. Актуальная версия лежит в [`PRIVACY_POLICY.md`](./PRIVACY_POLICY.md) и должна быть доступна по публичному URL (например, https://domgo.rs/privacy).
- Заполните форму **Data Safety**: мы собираем e‑mail, контент объявлений (текст/фото), список избранного и журналы ошибок (Sentry). Геолокация используется только по запросу пользователя при выборе города.
- В разделе **App Content → User Generated Content** опишите модерацию из `RULES.md` и контакты для жалоб.
- Загружайте только AAB (`bundleRelease`). APK оставляем для локального QA.
- Минимальный набор разрешений: `ACCESS_FINE_LOCATION`, `ACCESS_COARSE_LOCATION`, `READ_MEDIA_IMAGES`, `READ_MEDIA_VIDEO`, `INTERNET`, `VIBRATE`. Чувствительные права (`RECORD_AUDIO`, `SYSTEM_ALERT_WINDOW`, `WRITE_EXTERNAL_STORAGE`) удалены.
